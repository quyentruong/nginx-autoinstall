...
OPENSSL_VER=$(curl -sL https://github.com/openssl/openssl/releases 2>&1 | grep -E -o '/OpenSSL\_[0-9a-z_]+\.tar[.a-z]*' | awk -F "OpenSSL_" '/.tar.gz$/ {print $2}' | sed -e 's|.tar.gz||g' | head -n 2 2>&1)
NPS_VER=1.13.35.2
HEADERMOD_VER=0.33
